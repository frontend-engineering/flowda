...
export function processJsonschema(jsonschema: ResourceKey) {
  if (jsonschema.properties == null) throw new Error(`no properties, ${jsonschema.class_name}`)
  ...
  return Object.keys(props).reduce(
    (acc, cur) => {
      ...
      if ('model_name' in prop && !('reference_type' in prop)) {
        ...
        if (!ret.success)
          throw new Error(
            `association parse error, k:${cur}, prop: ${JSON.stringify(prop)}, error: ${ret.error.message}`,
          )
      ...
      if (!colParseRet.success)
        throw new Error(
          `column parse error, k:${cur}, prop: ${JSON.stringify(prop)}, error: ${colParseRet.error.message}`,
        )
